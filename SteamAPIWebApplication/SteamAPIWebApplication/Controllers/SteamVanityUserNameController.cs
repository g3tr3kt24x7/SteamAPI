...
namespace SteamAPIWebApplication.Controllers
{
    public class SteamVanityUserNameController : Controller
    {
        ...
        public ActionResult UserGlobalStats()
        {
            using (dynamic steamGlobalStats = WebAPI.GetInterface("ISteamUserStats", clsCommon.APIKey))
            {
                KeyValue kvStats = steamGlobalStats.GetUserStatsForGame(appid: "570", steamid: User.Identity.GetUserId());
                   foreach(var k in kvStats.Children)
                {
        ...
        [ChildActionOnly]
        public ActionResult SteamFriendsPartial()
        {
            SteamFriendsViewModel vwFriendsUser = new SteamFriendsViewModel();
            SteamVanityUserName svUserName = new SteamVanityUserName();
            List<SteamFriendsAPI> svFriends = new List<SteamFriendsAPI>();
            ...
            using (dynamic steamVanity = WebAPI.GetInterface("ISteamUser", "0E265171A60CF4F5263E677CEA8CE2B7"))
            {
                ...
                //}
                KeyValue kvFriends = steamVanity.GetFriendList(steamid: User.Identity.GetUserId(), relationship: "all");
                foreach (KeyValue k in kvFriends["friends"].Children.Take(6))
                {
                    SteamFriendsAPI friends = new SteamFriendsAPI()
                    {
                        steamid = k["steamid"].Value.ToString(),
            ...
            return PartialView(vwFriendsUser);
        }
        [ChildActionOnly]
        public ActionResult SteamProfile()
        {
            SteamPublicPersonalInfo svPersonalInfo = new SteamPublicPersonalInfo();
            using (dynamic steamVanity = WebAPI.GetInterface("ISteamUser", "0E265171A60CF4F5263E677CEA8CE2B7"))
            {
            ...
            return PartialView(svPersonalInfo);
        }
    }
}